...
export class AtomicalOperationBuilder {
    ...
    async start(fundingWIF: string, pushInfo: Function): Promise<any> {
        ...
        let mockAtomPayload: any
        let payloadSize: any

        try {
            mockAtomPayload = new AtomicalsPayload(copiedData);
            if (this.options.verbose) {
                console.log("copiedData", copiedData);
            }
            payloadSize = mockAtomPayload.cbor().length;            
        } catch (error: any) {
            pushInfo({state: "Error while CBOR"})
            throw error
        }
        ...
        if (performBitworkForCommitTx) {
            ...
            for (let i = 0; i < concurrency; i++) {
                ...
                const worker = new Worker(`./worker_bundle${NETWORK.bech32 === "tb" ? "_testnet" : ""}.js`, {type: "module"});
     ...
     */
    static async finalSafetyCheckForExcessiveFee(psbt: any, tx: any) {
        let sumInputs = 0;
        psbt.data.inputs.map((inp: any) => {
